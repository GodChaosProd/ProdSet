require "/scripts/vore/ps-profiles.lua"
...
function die()
	world.spawnProjectile( "ps-cleanser" , mcontroller.position(), entity.id(), {0, 0}, true )
	if eaten then
...
function update(dt)
	...
	if eaten and math.random(200) == 1 then
		if isDigest and math.random() < 0.1 * familiarity then
			npc.say( "^orange;" .. myProfile["plead"][familiarity][ math.random( #myProfile["plead"][familiarity] ) ] )
			plead = true
		else
